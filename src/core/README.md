...
- Interval helpers: `spanOf(segments: readonly Window[]): Window | null` and
...
| `spans.ts` | `spanOf`, `intersectionOf`, and the provenance sweep with exact sources per merged span |
...
end-exclusive epoch millisecond bounds. `spanOf` bridges disjoint segments;
...
Inputs are never mutated. The existing `Span` type names axis choices only.
